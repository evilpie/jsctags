...
function fastDispatchStm(n) {

  function fastDispatchExp(n) {
    switch (opArity[n.type]) {
    case UNARY: case BINARY: case TERNARY: case MULTI:
      n.forEach(fastDispatchExp);
      return;

    case MULTI_CALL:
      fastDispatchExp(n[0]);
      n[1].forEach(fastDispatchExp);
      return;
    ...
    case FUN:
      fastDispatchStm(n.body);
      return;

    case MULTI_OI:
      n.forEach(function(prop) {
          fastDispatchExp(prop[0]);
          fastDispatchExp(prop[1]);
        });
      return;
    }
  }
  ...
  if (!n) return;
  if (n.evalStm) return; // 2+ nodes can have the same n.kreg, don't redo work.

  switch(n.type) {
  case SCRIPT:
    n.funDecls.forEach(function(fd) { fastDispatchStm(fd.body); });
    break;

  case SEMICOLON:
    n.evalStm = function(fr) {
      var ans = evalExp(this.expression, fr); //this.expression.evalExp(fr);
      ans.v = this.kreg;
      return ans;
    };
    fastDispatchExp(n.expression);
    break;

  case BLOCK: case IF: case SWITCH: case CASE: case DEFAULT: 
  case FOR: case DO: case WHILE: case FINALLY:
    n.evalStm = function(fr) { return new Ans(this.kreg, fr); };
    break;

  case FOR_IN:
    n.evalStm = function(fr) {
      var av = BOTTOM, ans = evalExp(this.object, fr), errval = ans.err;
      ans.v.forEachObj(function(o) {
          if (o.hasNumericProp()) av = avjoin(av, ANUM);
          if (o.hasStringProp()) av = avjoin(av, ASTR);
        });
      ans.v = av;
      ans = evalLval(this.iterator, ans);
      errval = maybeavjoin(errval, ans.err);
      ans.v = this.kreg;
      return ans;
    };
  ...
  case TRY: // FIXME: doesn't handle guarded catch
    n.evalStm = function(fr) { return new Ans(this.kreg, fr); };
    n.catchClauses.forEach(function(clause) {
        clause.evalStm = function(fr) { return new Ans(this.block, fr); };
        fastDispatchStm(clause.block);
      });
    break;

  case THROW:
    n.evalStm = function(fr) {
      var ans = evalExp(this.exception, fr);
      ans.err = maybeavjoin(ans.err, ans.v);
      ans.v = this.kreg;
      return ans;
    };
    fastDispatchExp(n.exception);
    break;

  case RETURN:
    fastDispatchExp(n.value);
    break;
...
function evalFun(fn, args, withNew) {
  ...
  function stmThrows(n, errav) {
    if (n) {
      if (n.type === CATCH) frameSet(fr, n.exvar, errav);
      w.push(n);
    }
    else
