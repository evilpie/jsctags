...
function fixStm(n) {
  ...
  default:
    throw new Error("fixStm: unknown case: " + n.type + " " + n.length);
  }
  return n;
}
...
Aobj.prototype.hasNumericProp = function() {
...
Aval.prototype.toType = function() {
  ...
  var basetypes = {1 : "number", 2 : "string", 4 : "boolean", 8 : "undefined"};
  while (i <= 8) {
...
function initCoreObjs() {
    ...
    var _Array = (function () {
        ...
        return function(args, h, withNew) {
          ...
          retval.forEachObj(function (o) {
              h = o.updateProto(apav);
              h = o.updateProp("length-", ANUM);
            });
  ...
  (function () {
    ...
    // Error
    function _Error(args, h) {
      args[0].forEachObj(function (o) {
          h = o.updateProto(epav);
          h = o.updateProp("message-", ASTR);
        });
      return new Ans(args[0], undefined, h);
    }
  ...
  (function () {
    ...
    // RegExp
    function _RegExp(args, h) {
      args[0].forEachObj(function (o) { h = o.updateProto(rpav); });
      return new Ans(args[0], undefined, h);
    }
...
function addSummary(n, args, h, retval) {
  var addr = n.addr;
  // for level1 we use {} instead of an Array b/c it's sparse.
  ...
  for (var h in sums) {
    if (sums.hasOwnProperty(h)) {
      ...
      insouts.forEach(function(inout) {
          ...
          for (var i = 0; i < pl; i++)
            pjoins[i] = avjoin(pjoins[i], ins[i + 1] || AUNDEF/*arg mismatch*/);
          if (n.withNew)
            retjoin = avjoin(retjoin, ins[0]);
          else if (aveq(BOTTOM, inout[1])) // fun has no return => returns undef
            retjoin = avjoin(retjoin, AUNDEF);
          else
            retjoin = avjoin(retjoin, inout[1]);
        });
    }
  }
  pjoins.forEach(function(p, i, pjs) { pjs[i] = p.toType(); });
...
// node, answer, Aval or undefined -> answer
...
function evalLval(n, ans, oldlval) {
  ...
  function updateObjsProp(avobjs, pname, pval) {
    ...
    avobjs.forEachObj(function(o) { h = o.updateProp(pname, pval); })
  }
  ...
  case DOT: case DOT_PROTO:
    print("evallval dot");
    if (n[0].type === IDENTIFIER) print("evalLval/dot " + n[0].name);
...
function evalExp(n, fr, h) {
  ...
  case NULL: // don't distinguish null and undefined
    return new Ans(AUNDEF, fr, h);

  case REGEXP:
    var readdr = n.addr, newre = heap[readdr];
    if (!newre) {
      h = ++timestamp;
      heap[readdr] = newre = new Aobj({proto:regexp_prototype_av});
    }
    return new Ans(new Aval(NOBASEVALS, readdr), fr, h);
  ...
  case PLUS_ASSIGN:
    ...
    ans = evalExp(n[0], fr, h);
    return evalLval(n[0], evalExp(n[1], fr, h), ans.v);
  ...
  case HOOK:
    ans = evalExp(n[0], fr, h);
    ans1 = evalExp(n[1], ans.fr, ans.h);
    ans2 = evalExp(n[2], ans1.fr, ans1.h);
  ...
  case OBJECT_INIT:
    print("evalExp/object_init");
    var objaddr = n.addr, newobj = heap[objaddr];
    if (!newobj) {
      h = ++timestamp;
      heap[objaddr] = newobj = new Aobj({proto:object_prototype_av});
    }
    n.forEach(function(pinit) {
        ans = evalExp(pinit[1], fr, h);
        fr = ans.fr;
        h = newobj.updateProp(pinit[0].name, ans.v);
        errval = maybeavjoin(errval, ans.err);
      });
    ans = new Ans(new Aval(NOBASEVALS, objaddr), fr, h);
    ans.err = errval;
    return ans;
  ...
  case ARRAY_INIT:
    ...
    if (!newarray) {
      h = ++timestamp;
      heap[arrayaddr] = newarray = new Aobj({proto:array_prototype_av});
    }
    n.forEach(function(elm, i) {
        ans = evalExp(elm, fr, h);
        fr = ans.fr;
        h = newarray.updateProp(i + "-", ans.v);
        errval = maybeavjoin(errval, ans.err);
      });
    ans = new Ans(new Aval(NOBASEVALS, arrayaddr), fr, h);
    ans.err = errval;
    return ans;
  ...
  case DOT_PROTO:
    ...
    ans.v.forEachObj(function(o) {
        ...
        if (!clos) {
          ...
          h = o.updateProp("prototype-", proto);
          av = avjoin(av, proto);
        }
      });
  ...
  case NEW_WITH_ARGS:
    ...
    if (nt === CALL) {
      ...
      rator.forEachObj(function(o) {
          var clos = o.getFun();
          if (!clos) return; 
          ans = evalFun(clos, rands, h, false);
      ...
      // FIXME: the object here and at OBJECT_INIT and ARRAY_INIT doesn't have
      // to be created during the analysis, it can be created beforehand.
...
function evalStm(n, fr, h) {
  ...
  case CATCH:
    ...
    return new Ans(n.block, fr, h);
    
    // case FUNCTION: // fix for fun decls in blocks

    // case WITH: 

  case SEMICOLON: // handled in evalFun. Must be here too b/c of toplevel only.
    ans = evalExp(n.expression, fr, h);
    return new Ans(n.kreg, ans.fr, ans.h);

    
  default:
    throw new Error("evalStm: unknown case: " + n.type + " " + n.length);
  }
}

// FIXME: the set Seen is only useful among pending calls to evanFun, not calls
// that have returned already. Pass it as arg instead of having a global one &
...
function evalFun(fn, args, h, withNew) {
  ...
  if (fn.builtin) return fn.body(args, h, withNew);
  ...
  while (w.length !== 0) {
    ...
    case THROW:
      ans = evalExp(n.exception, fr, h);
      fr = ans.fr;
      h = ans.h;
      w.push(n.kreg);
      stmThrows(n.kexc, maybeavjoin(ans.v, ans.err));
      break;

    case FOR_IN:
      var av = BOTTOM;
      ans = evalExp(n.object, fr, h);
      if (ans.err) stmThrows(n.kexc, ans.err);
      ans.v.forEachObj(function(o) {
  ...
  ans = new Ans(retval, undefined, h);
...
function evalToplevel(tl) {
  ...
  w = [];
  fr = {};
  h = timestamp; // timestamp is > 0 b/c of initCoreObjs
  initDeclsInHeap(tl);
  ...
  while (w.length !== 0) {
    n = w.pop();
    if (!n) break; // end of toplevel reached
    if (n.type === RETURN)
  ...
  tl.funDecls.forEach(function(fd) {
      if (!existsSummary(fd)) {
        ...
        ans = evalFun(fd, buildArray(fd.params.length + 1, BOTTOM), h, false);
        h = ans.h;
      }
    });
...
function cfa2(ast) {
  ...
  if (commonJSmode) { // create the exports object
    ...
    print("created exports obj " + (!!exportsObj.obj));
  }
  labelStm(ast);
  tagVarRefsStm(ast, [], []);
