...
function initCoreObjs() {
  ...
  attachMethod(op, "hasOwnProperty",
               function(args, h) { return new Ans(ABOOL, undefined, h); });
  // Object.prototype.toString
  attachMethod(op, "toString",
               function(args, h) { return new Ans(ASTR, undefined, h); });
  ...
  attachMethod(o_p, "toString",
               function(args, h, withNew) {
                 // if (withNew) record an error
                 return new Ans(ASTR, undefined, h);
               }); 

  // Object
  function _Object(args, h) {
    var arg = args[1];
    if (!arg) {
      args[0].forEachObj(function (o) { h = o.updateProto(opav); });
...
function addSummary(n, args, h, retval) {
  ...
  //print("summary: " + n.fname.name + " " + h + " " + retval.avshow());
    ...
    for (h in summaries[addr]) {
      ...
      if (summaries[addr].hasOwnProperty(h)) {
        ...
        insouts.forEach(function(inout) {
            summ += "   ";
            inout[0].forEach(function(av) {summ += (av.avshow() + ", ");});
            summ += (":: " + inout[1].avshow());
            summ += "\n"
              });
        summ += ">";
...
function evalExp(n, fr, h) {
  ...
  // ans.v is an Aval denoting the object(s) whose property we access
  function evalPropertyAccess(ans, prop /* string */) {
    var h = ans.h, av = BOTTOM, av2, ans2;
  ...
  case DOT_PROTO:
    ...
    ans.v.forEachObj(function(o) {
        ...
        if (!clos) {
          ...
          updateHeapObjProp(o, "prototype-", proto);
          av = avjoin(av, proto);
        }
      });
    ans2 = new Ans(av, ans.fr, h);
  ...
  case INDEX:
    ...
    if ((pt === NUMBER) || (pt === STRING))
      return evalPropertyAccess(ansobj, prop.value.toString() + "-");
    ...
    if (avprop.hasNum) ans = evalPropertyAccess(ansobj, "-number");
    if (avprop.hasStr) ans2 = evalPropertyAccess(ansobj, "-string");
    ans.v = avjoin(ans.v, ans2.v);
    ans.thisav = avjoin(ans.thisav, ans2.thisav);
    return ans;
    
  case DOT:
    return evalPropertyAccess(evalExp(n[0], fr, h), n[1].name);
