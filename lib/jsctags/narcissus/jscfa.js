...
function funToType(n, seenObjs) {
  ...
  if ((n.withNew) && !n.hasReturn) {
    outtype = insjoin[0].toType(seenObjs);
...
function searchPending(n, args) {
  ...
  for (var i = bucket.length - 1; i >=0; i--)
    if (arrayeq(aveq, args, bucket[i][0])) {
      //print("sp " + bucket.length + " " + hash(args) + " " + bucket[i][1] + " " + n.addr);
      return bucket[i];
    }
  return false;
}
...
function addPending(n, elm) {
  ...
  if (!bucket)
    ...
    bucket.push(elm);
    //print("addp " + elm[1] +" "+ h + " " + bucket.length+" "+n.addr);
  }
}
...
function evalFun(fn, args, withNew) {
  ...
  //print("ef frame " + fn.addr+" "+timestamp+" "+depth);
  ...
  while(true) {
    try {
      ...
      if (!p2ts) {
        //print("nf " + timestamp + " " + args.length);
        ...
        //print("raytrace here " + timestamp + " :: " + p2ts + " " + arrayeq(aveq, args, pelm2[0]) +" "+ pelm2[2]);
      ...
      if (e.depth === depth) {
        //print("raytrace loop " + timestamp + " :: " + p2ts +" "+fn.fname.name);
