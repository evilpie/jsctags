...
function Aval(base, objaddr) {
  this.base = base;
  this.objs = [];
  if (objaddr !== undefined) {
    console.log("aval obj " + objaddr);
    this.objs.push(objaddr);
  }
}

// fun takes an abstract object
Aval.prototype.forEachObj = function(fun) {
  //console.log("feo " + this.objs.length);
...
function avjoin(v1, v2) {
  if (v2 === undefined) console.log("avjoin undef " + v1.toSource());
...
function updateHeapObjProp(o, prop, newv) {
  var oldv = o[prop];
  console.log("new property " + prop +" "+ newv.toSource());
  if (oldv === undefined) {
    o[prop] = newv;
    return ++timestamp;
  }
...
function updateHeapAv(addr, newv) {
  ...
  if (oldv === undefined) console.log("updateheapav undef");
  newv = avjoin(newv, oldv);
  heap[addr] = newv;
  if ((oldv.base !== newv.base) || (oldv.objs.length !== newv.objs.length))
    return ++timestamp;
...
function evalLval(n, ans) {
  ...
  case IDENTIFIER:
    ...
    case STACK:
      var vname = n.name;
      fr[vname] = avjoin(fr[vname], ans.v);
      // if (aveq(fr[vname], BOTTOM)) // FIXME: record unbound variable
      break;      
    case HEAP:
  ...
  case DOT:
    ...
    ans2.v.forEachObj(function(o) {
        h = updateHeapObjProp(o, prop, ans.v);
      });
    break;
...
function evalExp(n, fr, h) {
  ...
  case IDENTIFIER:
    switch (n.kind) {
    case STACK:
      // if (aveq(fr[n.name], BOTTOM)) // FIXME: record error, unbound variable
      return new Ans(fr[n.name], fr, h);
    case HEAP:
      // if (aveq(heap[n.addr], BOTTOM)) // FIXME: record unbound variable
  ...
  case CALL:
  case NEW_WITH_ARGS:
    console.log("nwa1 n[0] type " + n[0].type);
    ...
    if (nt === CALL) {
      ...
      rator.forEachObj(function(o) {
          if (o === undefined) console.log("nwa gotcha");
          var clos = o.getFun();
          if (!clos) return; 
      ...
      console.log("new obj addr " + objaddr);
      var av = new Aval(NOBASEVALS, objaddr);
      rands[0] = av;
      //console.log("nwa3");
      //console.log(rator.objs);
      // FIXME: record error if rator contains base vals and non-functions
      rator.forEachObj(function(o) {
          //console.log("nwa3.01");
          var clos = o.getFun();
  ...
  default:
    console.log("evalExp unhandled case: " + (nt === FUNCTION));
...
function evalFun(fn, args, h) { 
  ...
  console.log("funid: " + fn.fname.name +" "+ fn.addr);
  ...
  script.varDecls.forEach(function(vd) { fr[vd.name] = BOTTOM; });
  // bind the fun names in the frame.
  script.funDecls.forEach(function(fd) {
      fr[fd.fname.name] = new Aval(NOBASEVALS, fd.addr);
    });
...
function evalToplevel(tl) {
  ...
  // bind a non-init`d var to bottom, different from assigning undefined to it.
  tl.varDecls.forEach(function(vd) { fr[vd.name] = BOTTOM; });
  // bind the fun names in the frame.
  ...
  showSummaries();
  //console.log("last heap: " + h + "::" + timestamp);
}
