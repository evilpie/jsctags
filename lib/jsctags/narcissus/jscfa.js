 ...
 *
 * - fix summaries in evalFun to use all arguments
 *
...
function aveq(v1, v2) {
  if (v1 === UNKNOWN || v2 === UNKNOWN) return v1 === v2;
...
function evalExp(n, fr, h) {
  ...
  case CALL:
  case NEW_WITH_ARGS:
    var av, rands = [undefined /* reserved for THIS */], retval = BOTTOM, rator;
    ...
    if (nt === CALL) {
      //FIXME: bind rands[0] to the global onject
      ...
      rator.forEachObj(function(o) {
          ...
          // FIXME: if retval is bottom, turn it to undefined
          h = ans.h;
        });
    }
    else {
