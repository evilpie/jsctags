...
function funToType(n) {
  var addr = n.addr, summary = summaries[addr];
  if (summary.ts === 0) return "function"; // the function was never called
...
// Because of this function, we never get undefined by reading from fr or heap.
// Must be run after initGlobals.
...
function evalExp(n, fr) {
  ...
  case REGEXP:
    ...
    return new Ans(new Aval(NOBASEVALS, readdr), fr);
