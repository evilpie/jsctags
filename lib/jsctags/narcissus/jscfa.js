...
Aobj.prototype.toType = function(seenFuns) {
  if (this[1]) return funToType(this[1], seenFuns);
  ...
  if (types.length === 1) {
    if (types[0] === "Array") return this.toArrayType(seenFuns);
    return types[0];
  }
  return ("<" + types.join(" | ") + ">");
};
...
Aobj.prototype.getProp = function(prop) {
  ...
  var retval, av, notfoundinsomechain = false;
...
// Takes an optional array for cycle detection.
Aval.prototype.toType = function(seenFuns) {
  ...
  this.forEachObj(function (o) { types.push(o.toType(seenFuns)); });
...
function funToType(n, seenFuns) {
  if (seenFuns) {
    if (seenFuns.member(function(f) { return f === n; }))
      return "any";
    else
      seenFuns.push(n);
  }
  ...
  for (var i = 1, len = insjoin.length; i < len; i++)
    instypes[i - 1] = insjoin[i].toType(seenFuns);

  outtype = n.withNew ?
    insjoin[0].toType(seenFuns) :
    summary.type[1].toType(seenFuns);

  if ((n.withNew) && !n.hasReturn) {
    outtype = insjoin[0].toType(seenFuns);
    ...
    // In this case, the global obj in the result type must change to void.
    if (/Global Object/.test(outtype))
      outtype = outtype.replace("Global Object", "void");
  }
  else
    outtype = summary.type[1].toType(seenFuns);
...
function evalFun(fn, args, withNew) {
  ...
  while(true) {
    try {
      ...
      // from growing too much. It may be that even w/out it the stack size is
      // still manageable. If so, change back to the simpler implementation.
...
function cfa2(ast) {
  ...
  try {
    ...
    print("timestamp: " + timestamp +
          ", prot " + uprot +
          ", prop " + uprop +
          ", hvar " + uvar); // 1155 for v8:crypto and v8:splay
    print(e.message);
    throw e;
  }
}
