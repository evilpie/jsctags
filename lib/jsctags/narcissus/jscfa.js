 ...
 *
 * - function return: consider function foo(x,y,z) {if (z) return x+y;}
 *   This may return undefined if the else branch is taken but my current
 *   analysis won't see that.
 *
...
// An abstract object is an array whose 1st elm contains the address of its 
// prototype in the heap, its 2nd elm may contain a function node.
// Non-special properties aren't array elms, they are normal properties.
function Aobj() {
  this.push(undefined);
  this.push(undefined);
}
...
function avjoin(v1, v2) {
  ...
  return new Aval(v1.base | v2.base);
}
...
// function node -> abstract object
// used in the beginning of the analysis, to fill in the heap.
function makeFunObj(n) {
  var ao = new Aobj();
  //FIXME: set the prototype ao[0] to sth
  ao[1] = n;
  return ao;
}
...
function aplus(v1, v2) {
  if (v1 === UNKNOWN || v2 === UNKNOWN) return new Aval(3);
  if (((v1.base | v2.base) & 2) === 0) return ANUM;
...
function evalExp(n, fr, h) {
  ...
  case CALL:
    ...
    rator.forEachObj(function(o) {
        var clos = o.getFun();
        if (!clos) return; // FIXME: throw error if a non-function flows here?
...
function evalToplevel(tl) {
  ...
  fdecls.forEach(function(fd) {
      ...
      heap[objaddr] = makeFunObj(fd);
      av = new Aval();
      av.addObj(objaddr);
      if (fn.kind === HEAP) heap[fn.addr] = av;
      fr[fn.name] = av;
    });
