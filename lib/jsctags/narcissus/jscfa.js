...
// map here: mutate instead of creating a new array
Array.prototype.maph = function(fun) {
...
function fixExp(n) {
  ...
  case MULTI_OI:
    n.maph(function(prop) { fixExp(prop[0]); fixExp(prop[1]); })
    return;
  }
}
...
function fixStm(n) {
  ...
  case TRY: // turn the varName of each catch-clause to a node called exvar
    fixStm(n.tryBlock);
    n.catchClauses.maph(function(clause) {
...
function labelExp(n) {
  ...
  case MULTI_OI:
    n.maph(function(prop) { labelExp(prop[0]); labelExp(prop[1]); })
    return;
  }
}
...
function labelFun(n) {
  n.addr = ++count;
  n.params.maph( function(p) { p.addr = ++count; });
  labelStm(n.body);
}
...
function labelStm(n) {
  ...
  switch (n.type) {
  case SCRIPT:
    n.varDecls.maph(function(vd) {vd.addr = ++count;});
    n.funDecls.maph(labelFun);
...
function tagVarRefsExp(n, innerscope, otherscopes) {
  ...
  case UNARY: case TERNARY: case MULTI:
    n.maph(function(rand) { tagVarRefsExp(rand, innerscope, otherscopes); });
    return;

  case MULTI_CALL:
    tagVarRefsExp(n[0], innerscope, otherscopes);
    n[1].maph(function(arg) { tagVarRefsExp(arg, innerscope, otherscopes); });
    return;
  ...
  case MULTI_OI: 
    ...
    n.maph(function(prop) { tagVarRefsExp(prop[1], innerscope, otherscopes); });
    return;        
  }
}
...
function tagVarRefsFun(n, innerscope, otherscopes) {
  ...
  params.maph(function(param) { if (param.kind !== HEAP) param.kind = STACK; });
  // trim otherscopes
...
function tagVarRefsStm(n, innerscope, otherscopes) {
  ...
  case SCRIPT:
    ...
    fdecls.maph(function(fd) { tagVarRefsFun(fd, innerscope, otherscopes); });
    // tag the var refs in the body
    n.maph(function(stm) { tagVarRefsStm(stm, innerscope, otherscopes); });
    // tag formals
    vdecls.maph(function(vd) { if (vd.kind !== HEAP) vd.kind = STACK; });
    fdecls.maph(function(fd) { if (fd.kind !== HEAP) fd.kind = STACK; });    
    //trim inner scope 
...
function markConts(n, kreg, kexc) {
  ...
  switch (n.type) {
  case SCRIPT:
    n.funDecls.maph(function(fd) { markConts(fd.body, undefined, undefined); });
    // fall thru
  case BLOCK:
    n.kexc = kexc;
    len = n.length;
...
// A summary is a pair. The 1st elm is (function node, array of args, timestamp)
// The 2nd elm is (abstract value, timestamp)

// summaries is a 2-level hash table. The keys in level 1 are the addr`s of
// function nodes. The keys in level 2 are timestamps.
// It contains triples: args, abstract value, timestamp.
var summaries = {};
...
// function node, array of abstract values, timestamp -> Ans or false
// If there's a summary for this function call, return the result.
function searchSummary(n, args, h) {
  var bucket = summaries[n.addr][h], triple;

  for (var i = 0, len = bucket.length; i < len; i++) {
    triple = bucket[i];
    if (arrayeq(aveq, args, triple[0]))
      return new Ans(triple[1], undefined, triple[2]);
  }
  return false;
}
...
// TODO: make evalFun and evalToplevel use summaries
// TODO: use array utils wherever possible

// function node, array of abs. values, timestamp -> Ans (fr property not used)
function evalFun(fn, args, h) {
  var ans, i, n, len, params, fr, w /* workset */, retval, summ;
  summ = searchSummary(fn, args, h);
  if (summ) return summ;
  ...
  w.push(new Ans(fn.body.kreg, fr, h));
  while (w.length !== 0) {
    ans = w.pop();
    n = ans.v;
    fr = ans.fr;
  ...
  //FIXME: update summaries here
  return new Ans(retval, undefined, h);
}

function evalToplevel(tl) {
  var i, len, len2, fdecls = tl.funDecls, fdecl, args;

  timestamp = 0;
  for (i=0, len=fdecls.length; i<len; i++) {
    fdecl = fdecls[i];
    args = [];
    for (j=0, len2=fdecl.params.length; j<len2; j++) args.push(UNKNOWN);
    console.log(fdecl.name + " :: " +
                avshow(evalFun(fdecls[i], args, timestamp).v));
  }
}
...
function walkExp(n) {
  ...
  case OBJECT_INIT:
    n.maph(function(prop) { // prop.type === PROPERTY_INIT
...
function walkStm(n) {
  ...
  case TRY:
    walkStm(n.tryBlock);
    n.catchClauses.maph(function(clause) { // clause.varName is a string
  ...
  case CONST: // variable or constant declaration
    ...
    n.maph(function(vd) { walkExp(vd.initializer); });
    break;
  }
  return n;
}
