...
// of o2 are used for the "non-special" properties of o1. 
// 1st elm: the address of o1's prototype in the heap
// 2nd elm: may contain a function node.
// 3rd elm: may contain a constructor (a string).
function Aobj(specialProps) {
  ...
  this.push(specialProps.constructor); /* optional string */
}
...
Aval.prototype.initObj = function(addr) {
  this.objs.push(addr); // this.objs.push is empty initially
};
...
function evalExp(n, fr, h) {
  ...
  case NEW_WITH_ARGS:
    // there can be many possible constructors, n[0] can be an arbitraty expr.
    heap[n.addr] = new Aobj({constructor : }) // TODO continue here
