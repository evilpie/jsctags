...
/* FIXMEs:
 *
 * - fixStm rms all BREAK and CONTINUE nodes, the cfa would ignore them.
 *   It also changes the LABEL nodes to their statement.
 *   And the DEFAULT nodes to CASE nodes. (I ignore control-flow in switch)
 ...
 *
 * - all the Stm funs must handle: case LABEL:
 *
 * - fixStm turns all switches w/out branches to semi nodes w/ discrim.
 *
 * - do all stm funs handle empty blocks/scripts correctly?
 *
 * - rm all FUNCTION nodes from scripts, they're in funDecls
 *
 * - change break; to return; in switches that I do nothing after.
 */
...
// For every node N in the AST, add refs to the node that is normally exec'd
// after N and to the node that is exec'd if N throws an exception.
function markConts(n, kreg, kexc) {
  ...
  function markContsCatch(n, knocatch, kreg, kexc) {
    ...
    if (guard) {// Mozilla catch
      // The guard is of the form (varname if expr).
      // If expr is truthy, the body of the catch is exec'd w/ varname bound to
      // the exception.
  ...
  case FUNCTION:
    throw new Error("FIXME: Function nodes in stm context should go away, " +
                    "they don't help the cfa at all.");
...
function Ans(v, fr, h) {
  this.v = v; // evalExp puts abstract values here, evalStm sets of stacks
  this.fr = fr; // frame
  this.h = h; // heap
}

// FIXME: will be rewritten for fast dispatch
...
// stack of statements, frame, heap -> 
