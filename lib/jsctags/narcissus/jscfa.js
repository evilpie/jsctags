...
function funToType(n) {
  ...
  if ((n.withNew) && !n.hasReturn) {
    outtype = insjoin[0].toType();
    // If a fun is called both w/ and w/out new, assume it's a constructor.
    // If a constructor is called w/out new, this is bound to the global object.
    // In this case, the global obj in the result type must be changed to void.
    if (/Global Object/.test(outtype))
      outtype = outtype.replace("Global Object", "void");
  }
  else
    outtype = summary.type[1].toType();
