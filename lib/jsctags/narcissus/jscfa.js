...
function funToType(n) {
  ...
  var ps = n.params, pl = ps.length + 1 /* handle methods differently? */;
  var pjoins = buildArray(pl, BOTTOM), retjoin = BOTTOM;
  for (var h in sums) {
    var insouts = sums[h];
    insouts.forEach(function(inout) {
        inout[0].forEach(function(arg, i) {
            pjoins[i] = avjoin(pjoins[i], arg);
          });
