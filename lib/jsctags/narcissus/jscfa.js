...
function initCoreObjs() {
  ...
  attachMethod(go, "isNaN", toBool);
  attachMethod(go, "parseInt", toNum);
  ...
  (function () {
    ...
    attachMethod(s, "fromCharCode", toStr);
  })();
  ...
  (function () {
    ...
    attachMethod(m, "abs", toNum);
    attachMethod(m, "acos", toNum);
    attachMethod(m, "asin", toNum);
    attachMethod(m, "atan", toNum);
    attachMethod(m, "atan2", toNum);
    attachMethod(m, "ceil", toNum);
    attachMethod(m, "cos", toNum);
    attachMethod(m, "exp", toNum);
    attachMethod(m, "floor", toNum);
    attachMethod(m, "log", toNum);
    attachMethod(m, "max", toNum);
    attachMethod(m, "min", toNum);
    attachMethod(m, "pow", toNum);
    attachMethod(m, "random", toNum);
    attachMethod(m, "round", toNum);
    attachMethod(m, "sin", toNum);
    attachMethod(m, "sqrt", toNum);
    attachMethod(m, "tan", toNum);
  })();
  ...
  (function () {
    ...
    heap[bpaddr] = bp;
    attachMethod(bp, "toString", toStr);
    attachMethod(bp, "valueOf", toBool);
...
// function node, array of Aval -> [args, boolean] or false
function searchSeen(n, args) {
  var nseen = seen[n.addr];
  if (nseen.ts < timestamp) return false;
  return nseen.ins.member(function(elm) {
      return arrayeq(aveq, args, elm[0]);
    });
}
...
function addSeen(n, args) {
  ...
  if (nseen.ts === timestamp)
    nseen.ins.push([args, false/* means we haven't seen these args before */]);
  else {
    nseen.ts = timestamp;
    nseen.ins = [[args, false]];
  }
}
...
(function() {
  ...
  function _stackref(n, ans, oldlval) {
    ...
    frameSet(ans.fr, n, avjoin(frameGet(ans.fr, n), ans.v));
    return ans;
  }
  ...
  // function _globalref(n, ans, oldlval) {
  ...
  function _index(n, ans, oldlval) {
    ...
    if ((pt === NUMBER) || (pt === STRING)) {
      ...
      if (avprop.hasNum()) avobj.updateProp("-number", rval);
...
(function() {
  ...
  // function _globalref(n, fr) {
  //   //print("FIXME, global var: " + n.name);
  ...
  function _index(n, fr) {
    var prop = n[1], pt = prop.type, ansobj = evalExp[n[0].type](n[0], fr),
      errval = ansobj.err;
    // if [] notation is used with a constant, try to be precise.
    if ((pt === NUMBER) || (pt === STRING)) {
      ansobj.thisav = ansobj.v;
      ansobj.v = ansobj.v.getProp(prop.value.toString() + "-");
      ansobj.err = errval;
      return ansobj;
    }
    // else merge nums to a generic number and strings to a generic string.
    var ansprop = evalExp[prop.type](prop, ansobj.fr), avprop = ansprop.v;
    ...
    if (avprop.hasNum()) 
      ansobj.v = ansobj.thisav.getProp("-number");
    if (avprop.hasStr())
      ansobj.v = avjoin(ansobj.v, ansobj.thisav.getProp("-string"));
    return ansobj;
  }
  ...
  function _call(n, fr) {
    ...
    rands[0] = (ans.thisav ? ans.thisav : global_object_av);
    ...
    if (ans.thisav && aveq(retval, BOTTOM)) {
      print("unknown method line: " + n[0].lineno);
      if (n[0][0].type === IDENTIFIER) print("obj: " + n[0][0].name);
      if (n[0][1].type === IDENTIFIER) print("method: " + n[0][1].name);
    }
...
function evalFun(fn, args, withNew) {
  ...
  // return BOTTOM because the context of the call won't get the correct type
  // for the recursive function.
  // We want to see a call twice before we stop processing.
  var inSeen = searchSeen(fn, args);
  if (inSeen) {
    if (inSeen[1])
...
function evalToplevel(tl) {
  ...
  tl.funDecls.forEach(function(fd) {
      if (!existsSummary(fd)) {
        var any_args = buildArray(fd.params.length, BOTTOM);
        any_args.unshift(global_object_av);
        evalFun(fd, any_args, false);
      }
    });
