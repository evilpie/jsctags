...
Aobj.prototype.toArrayType = function() {
  var elmtype = BOTTOM, that = this;
  this.forEachOwnProp(function(p) {
      if (propIsNumeric(p)) elmtype = avjoin(elmtype, that[p]);
    });
...
function initCoreObjs() {
  ...
  (function () {
    ...
    attachMethod(sp, "split",
                 function(args, h) { return new Ans(osplitav, undefined, h); });
  ...
  (function () {
    ...
    // Date
    function _Date(args, h, withNew) {
      if (withNew) {
        args[0].forEachObj(function (o) { h = o.updateProto(dpav); });
        return new Ans(args[0], undefined, h);
      }
      return new Ans(ASTR, undefined, h);
    }
...
function evalExp(n, fr, h) {
  ...
  case UNARY_PLUS: case UNARY_MINUS: case INCREMENT: case DECREMENT: case BITWISE_NOT:
...
function evalFun(fn, args, h, withNew) {
  ...
  if (h < timestamp) print("mismatch: " + h + " " + timestamp);
  if ((h % 10000) === 0) print("big h: " + h);
  ...
  if (!fn.hasReturn) { retval = AUNDEF; print("no return: " + fn.fname.name); }
